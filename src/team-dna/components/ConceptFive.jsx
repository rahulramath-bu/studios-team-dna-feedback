...
export function ConceptFive({
  ...
  // Team readiness follows the generation lifecycle (same adapter as the
  // original waiting states): the team read exists only once it has been
  // generated. Enough completions alone just unlock "Generate anyway".
  const teamReady = readiness ? readiness.teamGenerated : true;
 ...
 *  design-system parts only. Narrow box: the count (how many have
 *  finished) on the product's own progress track, plus your way to your
 *  profile. Wide box: the locked insights, what unlocks them, and (once
 *  enough people are in) the Generate action with what it does. */
  ...
  return (
    <div className="fivex-stack" aria-label="Team profile">
      <div className="fivex-profile">
        ...
        <section className="fvc fvx-locked">
          <span className="fvx-locked-icon" aria-hidden="true">
            <BetterUpIcon name="Lock" size={18} />
          </span>
          <h2 className="fvc-title">
          ...
          {atThreshold && readiness?.onGenerate ? (
            <div className="fvx-locked-foot">
              ...
              <p className="fvx-locked-note">
                Builds the team profile from the {completed} who have finished.
                It updates as the rest finish.
              </p>
            </div>
          ) : (
            <p className="fvx-locked-note">
              You can generate early once {TEAM_READY_THRESHOLD} people have
              finished.
            </p>
          )}
        </section>
  ...
  return (
    <div className="fivex-stack" aria-label="Generating team profile">
      <section className="fvg" id="fvsec-generating" aria-busy="true">
        ...
        <p className="fvc-lead">
          Reading {total > 0 ? `${total} assessments` : 'the team\u2019s assessments'}{' '}
          into one team profile. This only takes a moment.
        </p>
...
/** What unblocks the team read, in the state's own terms: more people, or
 *  (once enough have finished) the manager generating it. */
function unlockPhrase(readiness) {
  const completed = readiness?.completedCount ?? 0;
  return completed >= TEAM_READY_THRESHOLD
    ? 'once the team profile is generated'
    : `once ${TEAM_READY_THRESHOLD} people finish`;
}

/** The hover read before the team exists: the score on its own terms. */
function soloRead(row, isOwn, name) {
  const subject = isOwn ? 'You lean' : `${name} leans`;
  if (row.score >= 60) return `${subject} ${row.trait.highLabel.toLowerCase()}.`;
  if (row.score <= 40) return `${subject} ${row.trait.lowLabel.toLowerCase()}.`;
  return `${isOwn ? 'You sit' : `${name} sits`} near the middle on this one.`;
}
  ...
  return (
    ...
    >
      <div className="fivex-profile">
        ...
        {/* Same card before and after the threshold: the person's five
            scores are theirs and always show. Only the team average tick
            (and the team-relative reads) wait for three completions. */}
        <section className="fvc">
          <h2 className="fvc-title">
            {teamReady
              ? isOwn
                ? 'You, next to the team'
                : `${name}, next to the team`
              : isOwn
                ? 'Where you land'
                : `Where ${name} lands`}
          </h2>
          <p className="fvc-lead">
            {teamReady
              ? bigFiveLead
              : `${isOwn ? 'Your' : `${name}\u2019s`} scores on the five traits. The team average appears on each line ${unlockPhrase(readiness)}.`}
          </p>
          <div className="fva-rows">
            {(teamReady
              ? model.rows
              : BIG_FIVE_TRAITS.map((trait) =>
                  model.rows.find((row) => row.trait.key === trait.key)
                ).filter(Boolean)
            ).map((row) => (
                ...
                <span className="fva-mini fvp-mini">
                  ...
                  <span className="fvp-me" style={{ left: `${row.score}%` }}>
                    ...
                    <span className="fvp-tip" role="tooltip">
                      ...
                      <span>
                        {teamReady
                          ? hoverRead(row, isOwn, name)
                          : soloRead(row, isOwn, name)}
                      </span>
                    </span>
                  </span>
                </span>
      ...
      <section className="fvg">
        ...
        <p className="fvc-lead">
          {teamReady ? (
            ...
            <>
              How {isOwn ? 'you prefer' : `${name} prefers`} to get work done.
              Each topic also shows where {isOwn ? 'you sit' : `${name} sits`}{' '}
              against the room {unlockPhrase(readiness)}.
            </>
          )}
        </p>
        <section className="fvc">
...
function ComparePicker({ scope, subjects, allSubjects, viewerId, readiness }) {
  ...
  // The vignette always shows: fall back to the whole roster (pending
  // people included) when fewer than three have finished.
  const hintPool =
    allSubjects.length >= 3 ? allSubjects : readiness?.roster ?? allSubjects;
            ...
            <ol className="fvx-steps">
              <li>
                {locked ? (
                  <>
                    <strong>
                      {completedCount} of {totalCount} are in.
                    </strong>{' '}
                    Once two have finished, tap any two faces in the bar above.
                  </>
                ) : anchor ? (
                  <>
                    <strong>
                      {anchor.id === viewerId ? 'You\u2019re' : `${firstName(anchor)} is`}{' '}
                      in. Tap one more face
                    </strong>{' '}
                    in the bar above.
                  </>
                ) : (
                  <>
                    <strong>Tap two faces in the bar above.</strong> They
                    slide together and the rest of the team steps back.
                  </>
                )}
              </li>
              <li>
                <strong>Get one read on the pair.</strong> Where they cover
                each other, where it rubs, and the agreement worth making.
              </li>
            </ol>
          </div>
          <PairHintVisual members={pickHintMembers(hintPool)} />
        </div>
      </section>
    </div>
  );
